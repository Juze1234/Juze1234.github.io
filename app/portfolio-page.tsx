...
const content = {
  en: {
    ...
    heroText: "Lead developer of Metro W.A.R. RP, with hands-on experience across gameplay scripting, model integration, level design, game design, and multiplayer economy systems.",
    ...
    contributions: [
      ...
      "Building the server's economy systems and progression logic",
    ...
    supportingText: "Additional demonstrations include the ghost train world event and an in-world resident passport. Underneath them sit the systems that make the metro feel lived in. Radiation zones eat a gas-mask filter while a repurposed wristwatch counts down what is left of it. Sanity erodes in the dark, collapses when a Dark One catches your eye, and strips the player of their held item and then their backpack as it falls. Handheld radios run through a server-side voice router that decides every tick who can hear whom: directly by distance, over a tuned frequency, or through someone else's radio left on speaker. The wider codebase also contains rental, access control, private storage, and persistent roleplay character cards.",
    ...
    about: [
      "I am the lead developer of Metro W.A.R. RP. I write the gameplay systems, bring models into the engine, build the levels, handle game design, and build the server's economy systems. There are two of us — the other developer mainly handles the world, the map, and economy balance. Over two years more than 900 people have gathered around the project, and at our last big test the server was full: all 80 players.",
  ...
  ru: {
    ...
    heroText: "Главный разработчик Metro W.A.R. RP с практическим опытом в программировании игровых механик, интеграции моделей, левел- и геймдизайне, а также в системах экономики многопользовательского сервера.",
    ...
    contributions: [
      ...
      "Реализация систем серверной экономики и логики прогрессии",
    ...
    supportingText: "Дополнительные демонстрации показывают мировое событие «поезд-призрак» и внутриигровой паспорт жителя. Под ними работают системы, из-за которых метро кажется обжитым. Заражённые зоны расходуют фильтр противогаза, а переделанные наручные часы отсчитывают, сколько его осталось. Рассудок тает в темноте и обваливается от взгляда Тёмного, а когда падает совсем низко, игрок роняет сначала то, что держит в руках, а потом и рюкзак. Рации работают через серверный маршрутизатор голоса, который каждый тик решает, кто кого слышит: напрямую по дистанции, по совпадающей частоте или через чужую рацию, оставленную на динамике. В проекте также есть аренда, контроль доступа, приватные хранилища и ролевые карточки персонажа, которые сохраняются между сессиями.",
    ...
    about: [
      "Я главный разработчик Metro W.A.R. RP. Пишу игровые механики, переношу модели в движок, занимаюсь уровнями и геймдизайном, делаю системы серверной экономики. Нас в команде двое — второй разработчик в основном отвечает за мир, карту и баланс экономики. За два года вокруг проекта собралось больше 900 человек, а на последнем крупном тесте сервер был забит полностью, все 80 игроков.",
