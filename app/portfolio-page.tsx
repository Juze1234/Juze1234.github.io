...
const content = {
  ...
  ru: {
    ...
    supportingText: "Дополнительные демонстрации показывают мировое событие «поезд-призрак» и внутриигровой паспорт жителя. Под ними работают системы, из-за которых метро кажется обжитым. Заражённые зоны съедают ресурс фильтра противогаза, а переделанные наручные часы отсчитывают, сколько его осталось. Рассудок тает в темноте, обваливается от взгляда Тёмного и на низких значениях отнимает у игрока сначала предмет из рук, а затем и рюкзак. Рации работают через серверный маршрутизатор голоса, который каждый тик решает, кто кого слышит: по дистанции напрямую, по совпадающей частоте или через чужую рацию, оставленную на динамике. В кодовой базе также есть аренда, контроль доступа, приватные хранилища и персистентные ролевые карточки персонажа.",
